...
//    private val adapterexpIncomeTracker by lazy { Adapter_Exp_Income_Tracker() }
    ...
    ): View? {  binding = DataBindingUtil.inflate(layoutInflater,R.layout.fragment_expensess__income_tracker,container,false)
        return binding.root
...
//        setData()
//        adapterexpIncomeTracker.setList(createGroupList)
...
//    }
    ...
    private fun GetExpensesincomeTracker() {
        adapterExpIncomeTracker= activity?.let { Adapter_Exp_Income_Tracker(it.applicationContext,ArrayList<Data>()) }!!
    ...
    private fun getObserver(getExpensesIncomeTracker: GetExpensesIncomeTracker)
    {
        if (getExpensesIncomeTracker!=null)
        {
            ...
            Toast.makeText(requireActivity(), "data$getExpensesIncomeTrackerList", Toast.LENGTH_SHORT).show()
